...
import { AddressForm, Address } from '../../../types';

interface Props {
  onClose: () => void;
  onUpdate: (data: AddressForm) => Promise<void>;
  clienteNome: string;
  endereco: Address;
  hasMultipleAddresses: boolean;
}

const ModalEditarEnderecoCliente: React.FC<Props> = ({ onClose, onUpdate, clienteNome, endereco, hasMultipleAddresses }) => {
  ...
  const [hasNumber, setHasNumber] = useState(endereco.number !== 'S/N' && endereco.number !== '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Erro ao atualizar endereço');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <h3 className="text-xl font-semibold text-gray-900">Editar Endereço</h3>
...
export default ModalEditarEnderecoCliente;
